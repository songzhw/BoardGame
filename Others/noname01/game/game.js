...
(function() {
    ...
    init: {
      init: function() {
        ...
        for (var i in config2) {
          switch (i) {
            case "cheat":
              lib.config.cheat = config2[i];
              break;
            case "confirmtouch":
              lib.config.confirmtouch = config2[i];
              break;
            case "auto_skill":
              lib.config.auto_skill = config2[i];
              break;
            case "auto_confirm":
              lib.config.auto_confirm = config2[i];
              break;
            case "enable_drag":
              lib.config.enable_drag = config2[i];
              break;
            case "hover_all":
              lib.config.hover_all = config2[i];
              break;
            case "hover_handcard":
              lib.config.hover_handcard = config2[i];
              break;
            case "duration":
              lib.config.duration = config2[i];
              break;
            case "hoveration":
              lib.config.hoveration = config2[i];
              break;
            case "character":
              lib.config.image.character = config2[i];
              break;
            // case 'card':lib.config.image.card=config2[i];break;
            // case 'card_font':lib.config.card_font=config2[i];break;
            case "background":
              lib.config.image.background = config2[i];
              break;
            // case 'background_stretch':lib.config.image.background_stretch=config2[i];break;
            case "background_filter":
              lib.config.image.background_filter = config2[i];
              break;
            case "dynamic_background":
              lib.config.image.dynamic_background = config2[i];
              break;
            case "ui_zoom":
              lib.config.ui_zoom = config2[i];
              break;
            case "show_name":
              lib.config.show_name = config2[i];
              break;
            case "show_replay":
              lib.config.show_replay = config2[i];
              break;
            case "show_playerids":
              lib.config.show_playerids = config2[i];
              break;
            case "show_pause":
              lib.config.show_pause = config2[i];
              break;
            case "show_auto":
              lib.config.show_auto = config2[i];
              break;
            case "right_info":
              lib.config.right_info = config2[i];
              break;
            case "long_info":
              lib.config.long_info = config2[i];
              break;
            case "only_fullskin":
              lib.config.only_fullskin = config2[i];
              break;
            case "hide_card_image":
              lib.config.hide_card_image = config2[i];
              break;
            case "show_wuxie":
              lib.config.show_wuxie = config2[i];
              break;
            case "show_stat":
              lib.config.show_stat = config2[i];
              break;
            case "show_scrollbar":
              lib.config.show_scrollbar = config2[i];
              break;
            // case 'handcard_scroll':lib.config.handcard_scroll=config2[i];break;
            case "mousewheel":
              lib.config.mousewheel = config2[i];
              break;
            case "fold_card":
              lib.config.fold_card = config2[i];
              break;
            case "touchscreen":
              lib.config.touchscreen = config2[i];
              break;
            case "threed_card":
              lib.config.threed_card = config2[i];
              break;
            case "blur_ui":
              lib.config.blur_ui = config2[i];
              break;
            case "vertical_scroll":
              lib.config.vertical_scroll = config2[i];
              break;
            case "hide_history":
              lib.config.hide_history = config2[i];
              break;
            case "right_sidebar":
              lib.config.right_sidebar = config2[i];
              break;
            case "right_click":
              lib.config.right_click = config2[i];
              break;
            case "sort":
              lib.config.sort = config2[i];
              break;
            case "reverse_sort":
              lib.config.reverse_sort = config2[i];
              break;
            case "paused":
              lib.config.paused = config2[i];
              break;
            case "animation":
              lib.config.animation = config2[i];
              break;
            case "title":
              lib.config.title = config2[i];
              break;
            case "intro":
              lib.config.intro = config2[i];
              break;
            case "theme":
              lib.config.theme = config2[i];
              break;
            case "layout":
              lib.config.layout = config2[i];
              break;

            case "gameconfig":
              lib.config.gameconfig = config2[i];
              break;
            case "modeconfig":
              lib.config.modeconfig = config2[i];
              break;
            case "appearence":
              lib.config.appearence = config2[i];
              break;
            ...
            case "forbidlist":
              lib.config.forbidlist = config2[i];
              break;
            case "autoskilllist":
              lib.config.autoskilllist = config2[i];
              break;
    ...
    element: {
      ...
      player: {
        ...
        chooseButton: function() {
          ...
          if (next.ai == undefined) next.ai = function() {
            return 1;
          };
    ...
    create: {
      ...
      arena: function() {
        ...
        switch (lib.config.ui_zoom) {
          case "极小":
            ui.window.style.zoom = 0.8;
            break;
          case "很小":
            ui.window.style.zoom = 0.9;
            break;
          case "较小":
            ui.window.style.zoom = 0.95;
            break;
          case "较大":
            ui.window.style.zoom = 1.05;
            break;
          case "很大":
            ui.window.style.zoom = 1.1;
            break;
          default:
            ui.window.style.zoom = 1;
        }
  ...
  HTMLDivElement.prototype.transform = function(transform) {
    ...
    for (var i in transform) {
      switch (i) {
        case "scale":
          str += "scale(" + transform[i] + ") ";
          break;
        case "rotate":
          str += "rotate(" + transform[i] + "deg) ";
          break;
      }
    }
  ...
  window.countGroups = function() {
    ...
    for (var i in lib.character) {
      switch (lib.character[i][1]) {
        case "wei":
          a++;
          if (lib.config.forbidsingle.contains(i)) sa++;
          break;
        case "shu":
          b++;
          if (lib.config.forbidsingle.contains(i)) sb++;
          break;
        case "wu":
          c++;
          if (lib.config.forbidsingle.contains(i)) sc++;
          break;
        case "qun":
          d++;
          if (lib.config.forbidsingle.contains(i)) sd++;
          break;
      }
    }
